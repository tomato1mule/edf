...
task_name=mug_task
pick_type=rim
task_name=${task_name}_${pick_type}_1_demo
demo_dir=./demo/$task_name.gzip
pick_config=./config/train_config/1_demo/train_pick.yaml
place_config=./config/train_config/1_demo/train_place.yaml
...
PYTHONHASHSEED=$seed python3 edf/train_place.py --visualize --save-plot --save-checkpoint --sample-dir="$demo_dir" --seed=$seed --train-config-dir=$place_config | tee logs/train/place_agent/output.txt
echo "##### Place Training Done #####"

sleep 5
echo "##### Begin Pick Training with Seed $seed #####"
mkdir -p logs/train/pick_agent
PYTHONHASHSEED=$seed python3 edf/train_pick.py --visualize --save-plot --save-checkpoint --sample-dir="$demo_dir" --seed=$seed --train-config-dir=$pick_config | tee logs/train/pick_agent/output.txt
