...
class TransporterAgent:
    ...
    def __init__(self, name, task, root_dir, device=1, n_rotations=36, load=False, crop_size = 16*14, pix_size = 0.00125, bounds = np.array([[0.4, 0.8],[-0.2, 0.2], [0., 0.4]]), H = 320, W = 320):
        self.name = name
        self.task = task
        self.total_steps = 0
    ...
    def get_sample(self, data):
        img, pick, place = data
        p0 = pick[0]
        p0_theta = (pick[1] + 2*np.pi)%(2*np.pi)
        p1 = place[0]
        p1_theta = (place[1] + 2*np.pi)%(2*np.pi) - p0_theta
    ...
    def train(self, data):
        ...
        """
        time_0 = time.time()
        img, p0, p0_theta, p1, p1_theta = self.get_sample(data)
        # print('img',img.shape)
        # print('p0',p0)
    ...
    def act(self, img, info=None, goal=None, return_output=False):  # pylint: disable=unused-argument
        ...
        p0_theta = argmax[2] * (2 * np.pi / self.n_rotations)
        ...
        if return_output is True:
            return (p0_pix, p0_theta), (p1_pix, p1_theta), (pick_conf, place_conf)
        else:
            return (p0_pix, p0_theta), (p1_pix, p1_theta)
