...
from edf.pybullet_env.env import BowlTask
from edf.utils import OrthoTransform

# PYTHONHASHSEED=0 python3 generate_demo_bowl.py --use-gui --file-name='bowl_task_rim.gzip' --pick-type='rim' --dont-save
if __name__ == '__main__':
    ...
    for seed in seeds:
        if cup_pose == 'arbitrary':
            task.reset(seed = seed, mug_pose='upright' if seed%2==0 else 'lying', mug_type='default', distractor=False, use_support=False)
        else:
            task.reset(seed = seed, mug_pose=cup_pose, mug_type='default', distractor=False, use_support=False)
