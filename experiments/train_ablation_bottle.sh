...
seed=0
task_name=bottle_task
pick_type=rim
task_name=${task_name}_${pick_type}
demo_dir=./demo/$task_name.gzip
pick_config=./config/agent_config/scalar_field/pick_agent.yaml
place_config=./config/agent_config/scalar_field/place_agent.yaml
...
PYTHONHASHSEED=$seed python3 edf/train_place.py --visualize --save-plot --save-checkpoint --sample-dir="$demo_dir" --seed=$seed --agent-config-dir=$place_config | tee logs/train/place_agent/output.txt
echo "##### Place Training Done #####"

sleep 5
echo "##### Begin Pick Training with Seed $seed #####"
mkdir -p logs/train/pick_agent
PYTHONHASHSEED=$seed python3 edf/train_pick.py --visualize --save-plot --save-checkpoint --sample-dir="$demo_dir" --seed=$seed --agent-config-dir=$pick_config | tee logs/train/pick_agent/output.txt
